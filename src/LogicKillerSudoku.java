...
public class LogicKillerSudoku {
     ...
     */
    public LogicKillerSudoku(KillerSudokuBoard board){
        this.board=board;
    }
    /**
     ...
     */
    public LogicKillerSudoku() {
        board = new BoardFiles().killerFiles("Killer1");
    }
    ...
    public boolean delete(int line, int column){
        board.setvalue(line-1,column-1,0);
        return true;
    }
     ...
     * @return ο πίνακας
     */
    public KillerSudokuBoard getBoard(){
        return board;
    }
    /**
    ...
    private boolean checker(int line, int column, int value){
        ...
        int l,c;
        for(j=0;j<9;j++){
            if(board.getvalue(line-1,j)==value){ return false;}
        }
        for(i=0;i<9;i++){
            if(board.getvalue(i,column-1)==value){ return false; }
        }
        l=(line-1) / 3;
        c=(column-1) / 3;
        for(i=(l*3);i<(l*3)+3;i++){
        ...
        int code=board.getAreaOfElement(line-1,column-1);
        if(!((board.getAreaCurrentSum(code)+value)<=board.getElementAreaSum(line-1,column-1))){
            return false;
        }
        return true;
    }
    ...
    public boolean insert(int line,int column,int value){
        ...
        if(value==board.getvalue(line-1,column-1)){
            return true;
        }
        if(checker(line,column,value)){
            board.setvalue(line-1,column-1,value);
            return true;
        }
        return false;
    }
     ...
     */
    public int getElement(int x, int y) { return board.getvalue(x,y); }
}
